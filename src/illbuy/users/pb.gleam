...
pub fn decode_to_authenticate_request(
  ...
  case binary {
    ...
    _ -> {
      ...
      case key.field_number {
        1 -> {
          use #(username, binary) <- result.try(decoding.decode_field(
            binary,
            key.wire_type,
            decoding.string_field_decoder,
          ))
          decode_to_authenticate_request(
            binary,
            AuthenticateRequest(..authenticate_request, username: username),
          )
        }
        2 -> {
...
pub type RegisterUserRequest {
  RegisterUserRequest(email: String, password: String)
}

pub const empty_register_user_request = RegisterUserRequest("", "")

pub fn encode_register_user_request(
...
pub fn decode_to_register_user_request(
  binary: BitArray,
  register_user_request: RegisterUserRequest,
) -> Result(RegisterUserRequest, String) {
  case binary {
    <<>> -> Ok(register_user_request)
    _ -> {
      use #(key, binary) <- result.try(decoding.read_key(binary))
      case key.field_number {
        1 -> {
          ...
          decode_to_register_user_request(
            binary,
            RegisterUserRequest(..register_user_request, email: email),
          )
        }
        2 -> {
          use #(password, binary) <- result.try(decoding.decode_field(
          ...
          decode_to_register_user_request(
            binary,
            RegisterUserRequest(..register_user_request, password: password),
          )
        }
        _ -> Error("Invalid field_number")
      }
...
pub const register_user_request_field_encoder = FieldEncoder(
  wire.Len,
  encode_register_user_request,
)

fn register_user_request_field_decoder() {
  FieldDecoder(wire.Len, decode_to_register_user_request(
    _,
    empty_register_user_request,
  ))
}
